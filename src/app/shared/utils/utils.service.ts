...
export class UtilsService {
  ...
  smoothScroll(element: string, e?, offsetHeader?: boolean, sectionOffset?: number) {
    if (e) { e.preventDefault(); }
    const sectionToScrollTo = document.querySelector(`#${element}`);
    let headerOffset;
    if (offsetHeader && !sectionOffset) {
      headerOffset = this.isMobile() ? 80 : 100;
    } else if (offsetHeader && sectionOffset) {
      headerOffset = this.isMobile() ? sectionOffset : 100;
    } else {
      headerOffset = 0;
    }

    const sectionTop = _.get(sectionToScrollTo.getBoundingClientRect(), 'top');
    const offset = sectionTop - headerOffset + window.pageYOffset;
